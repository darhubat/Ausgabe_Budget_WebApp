...
def daten_budgeteingabe():
    ...
    df['Thema'] = 'Kein Thema'
    budget_daten = df.groupby([df['Budgetmonat'].dt.year.rename('Jahr'), df['Budgetmonat'].dt.month.rename('Monat'), df['Thema']])['Budgetbetrag'].sum().reset_index()
...
def daten_ausgabeneingabe():
    ...
    ausgaben_daten = df.groupby([df['Datum'].dt.year.rename('Jahr'), df['Datum'].dt.month.rename('Monat'), df['Thema']])['Ausgabenbetrag'].sum().reset_index()
